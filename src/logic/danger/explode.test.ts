...
import { canBeExploded, canExplodeDueToProximity } from "./explode";

test("token cannot explode when index is out of bounds", () => {
  const state: GameState = deepCopy(initialState);
  expect(canExplodeDueToProximity(state.grid, -1)).toBeFalsy();
});
...
test("token cannot explode when index is empty", () => {
  ...
  expect(canExplodeDueToProximity(state.grid, index)).toBeFalsy();
});
...
test("token can explode when adjacent to explodable enemy token", () => {
  ...
  expect(canExplodeDueToProximity(state.grid, index)).toBeTruthy();
});
